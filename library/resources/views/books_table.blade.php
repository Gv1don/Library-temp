<div class="headTable">
    ...
    <form action="{{ route('create') }}" method="GET">
        @csrf
        <button name="type" value="Book" type="submit" class="btn btn-primary" style="margin-top: 30px; max-height: 40px; max-width: 200px">+ Добавить книгу</button>
    </form>
</div>
    ...
    <table class="table table-striped table-lg table-hover">
        <thead>
            <tr>
                ...
                <th class="text-center">Возрастной рейтинг</th>
        ...
        <tbody>
            @foreach ($table as $row)
                <tr>
                    ...
                    <td class="text-center">{{ $row->age_rating }}</td>
                    <td class="col-1">
                        <div class="dashboard">
                            <form action="{{ route('update') }}" method="GET">
                                @csrf
                                <button type="submit" value="{{ $row->book_id }}" class="btn btn-primary" name="id">
                            ...
                            <form action="{{ route('delete') }}" method="POST">
                                @csrf
                                <button type="submit" value="{{ $row->book_id }}" class="btn btn-outline-danger" name="id">
